...
update_progress() {
    ...
    local pct=$((CURRENT_WEIGHT * 100 / TOTAL_WEIGHT))
    echo "[PROGRESS] $pct"
}
...
install_base() {
    ...
    apt-get install -y -qq curl wget git unzip xz-utils >>"$LOG_FILE" 2>&1 || true
...
install_jdk() {
    ...
    if ! tar xzf /tmp/jdk17.tar.gz -C "$HOME_DIR" >>"$LOG_FILE" 2>&1; then
...
install_flutter() {
    ...
    if ! wget -q -O /tmp/flutter.tar.xz \
    ...
    if ! tar xf /tmp/flutter.tar.xz -C "$HOME_DIR" >>"$LOG_FILE" 2>&1; then
...
patch_flutter() {
    ...
    if [ -f "$SHARED_DART" ]; then
        sed -i "s/Platform.operatingSystem == 'linux'/Platform.operatingSystem == 'linux' || Platform.operatingSystem == 'android'/g" "$SHARED_DART" >>"$LOG_FILE" 2>&1 || true
    fi
...
install_android_sdk() {
    ...
    if ! wget -q -O /tmp/cmdline-tools.zip \
    ...
    mkdir -p "$HOME_DIR/android-sdk/cmdline-tools" >>"$LOG_FILE" 2>&1 || true

    if ! unzip -q /tmp/cmdline-tools.zip -d /tmp/cmdline-tools >>"$LOG_FILE" 2>&1; then
...
install_build_tools() {
    ...
    if [ -f "$HOME_DIR/android-sdk/cmdline-tools/latest/bin/sdkmanager" ]; then
        ...
        sdkmanager "build-tools;35.0.1" "platform-tools" "platforms;android-35" >>"$LOG_FILE" 2>&1 || true
    fi
...
configure_git() {
    ...
    git config --global user.name "Flutter Developer" >>"$LOG_FILE" 2>&1 || true
    git config --global user.email "dev@androdart.studio" >>"$LOG_FILE" 2>&1 || true
    git config --global init.defaultBranch main >>"$LOG_FILE" 2>&1 || true
...
main() {
    ...
    echo "  Setup Complete!"
    echo "========================================="
    [ -f "$HOME_DIR/jdk/bin/java" ] && echo "  JDK: Installed"
    [ -f "$HOME_DIR/flutter/bin/flutter" ] && echo "  Flutter: Installed"
